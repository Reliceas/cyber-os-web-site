...
export function NeonSnakeApp() {
  ...
  const dashTimeoutRef = useRef<NodeJS.Timeout>()
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const currentEasing = isDashing ? EASING_FACTOR * DASH_SPEED_MULTIPLIER : EASING_FACTOR
      ...
      for (let i = snake.length - 1; i >= 0; i--) {
        ...
        bodyGradient.addColorStop(0, `hsla(${segmentHue}, 100%, 80%, ${0.95 * progress + 0.05})`)
        bodyGradient.addColorStop(0.5, `hsla(${segmentHue}, 100%, 60%, ${0.85 * progress + 0.05})`)
        bodyGradient.addColorStop(1, `hsla(${segmentHue}, 100%, 45%, ${0.7 * progress + 0.05})`)
      ...
      if (isDashing) {
        for (let i = 0; i < 5; i++) {
          ...
          ctx.beginPath()
          ctx.arc(particle.x + offsetX, particle.y + offsetY, Math.random() * 3 + 1, 0, Math.PI * 2)
  ...
  return (
    <div ref={containerRef} className="h-full bg-black relative overflow-hidden">
      ...
      >
        <div className="text-sm font-mono" style={{ color: `hsl(${hue}, 100%, 60%)` }}>
